...
func _input(event: InputEvent) -> void:
    ...
    match state:
        ...
        6:
            if event.is_action_pressed('jump'):
                $UnoCamera/CanvasLayer/Text3.visible = false
                $UnoCamera/CanvasLayer/Text4.visible = true
                currentText = $UnoCamera/CanvasLayer/Text4.text;
                $UnoCamera/CanvasLayer/Text4.text = '';
                charTimer = TIME_BETWEEN_CHARS;
                hideLabel($UnoCamera/CanvasLayer/Continue)
                state += 1
        8:
            if event.is_action_pressed('jump'):
                hideLabel($UnoCamera/CanvasLayer/Text4)
...
func _process(delta: float) -> void:
    ...
    match state:
        ...
        7:
            if charTimer <= 0:
                $UnoCamera/CanvasLayer/Text4.text += currentText[0]
                charTimer = TIME_BETWEEN_CHARS
                currentText = currentText.substr(1)
                if currentText.length() == 0:
                    state += 1
